...
为链接程序，您需要链接已构建的库。或者，您可以使用仅需头文件（header-only）的配置，只需在任意一个新建或现有源文件中包含此头文件：
...
在本库中，&lt;<ref_array>&gt;、&lt;<ref_object>&gt; 和 &lt;<ref_string>&gt; 类型分别用于容纳 JSON 数组、对象和字符串，而 &lt;<ref_value>&gt; 类型是一种特殊变体，可容纳任何 JSON 元素。此处我们构造一个空对象，然后插入上述元素：</ref_value></ref_string></ref_object></ref_array>
...
通过合理使用适当的内存资源、解析器实例及缓冲区大小的计算上限，可在不进行__any__（任何）动态内存分配的情况下解析和检查 JSON。后续章节将对此进行更详细探讨。
...
这允许我们可以使用库函数 &lt;<ref_value_from>&gt; 从类型生成一个 &lt;<ref_value>&gt;：</ref_value></ref_value_from>
