...
可以使用 &lt;&gt; 构造一个空数组，而不会产生任何内存分配。也可以显式指定一个 &lt;<ref_storage_ptr>&gt;：</ref_storage_ptr>
...
当 &lt;<ref_array>&gt; 被格式化输出到 {std_ostream} 时，结果是一个有效的 JSON。也就是说，该数组将按照 JSON 规范，以方括号和逗号分隔的值列表的形式输出。</ref_array> 当将 &lt;<ref_array>&gt; 格式化输出到 {std_ostream} 时，结果是一个符合 JSON 规范的有效 JSON 文本：数组将以方括号包围，并以逗号分隔其中的值。</ref_array>
