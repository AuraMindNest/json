...
[WARNING]
====
请勿创建类型为 {std_initializer_list} 的变量。这可能导致临时对象在初始化列表被使用之前就被销毁。
====

在所有情况下，通过初始化列表构造的 &lt;<ref_object>&gt;、&lt;<ref_array>&gt; 或 &lt;<ref_value>&gt; 所拥有的 &lt;<ref_storage_ptr>&gt; 都会递归地传播给每个元素。</ref_storage_ptr></ref_value></ref_array></ref_object>
